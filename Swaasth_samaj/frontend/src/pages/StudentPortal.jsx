...
const TAB_CONFIG = [
  ...
  { key: 'lounge',         icon: '🎓', label: "Student Hub" },
...
export default function StudentPortal() {
  ...
  const isDoctor = false; // Disabled doctor-only features for student portal
  ...
  const fetchQuestions = useCallback(async () => {
    try {
      const { data } = await axios.get('/api/questions?status=open&limit=50');
      ...
      <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '0 24px 60px', display: 'flex', gap: '28px' }}>
        ...
        <aside style={{
          ...
          <div style={{ background: 'white', borderRadius: '20px', padding: '28px 20px', boxShadow: '0 4px 20px rgba(0,0,0,0.06)', border: '1px solid rgba(30,94,255,0.08)', marginBottom: '16px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '0.8rem', color: '#6366f1', fontWeight: 700, background: 'rgba(99,102,241,0.1)', padding: '3px 12px', borderRadius: '100px', display: 'inline-block', marginBottom: '8px' }}>
              🎓 Verified Med. Student
            </div>
            {user.college && <div style={{ color: '#64748b', fontSize: '0.8rem', marginBottom: '4px' }}>📍 {user.college}</div>}
            {user.yearOfStudy && <div style={{ color: '#94a3b8', fontSize: '0.78rem' }}>{user.yearOfStudy}</div>}
          </div>
